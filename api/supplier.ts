...
export const getSupplierTypes = async () => {
  try {
    ...
    if (error instanceof AxiosError) {
      return (
        error.response?.data || {
          message: "An error occurred while fetching supplier types",
          status: false,
        }
      );
    } else {
...
export const becomeSupplier = async (payload: BecomeSupplier) => {
  try {
    ...
    if (error instanceof AxiosError) {
      return (
        error.response?.data || {
          message: "An error occurred during supplier registration",
          status: false,
        }
      );
    } else {
...
export const inviteUserToSupplier = async (payload: InviteUserPayload) => {
  try {
    const response = await AxiosClient.post(
      ENDPOINTS.supplier.INVITE_USER,
      payload
    );
    return response.data;
  } catch (error) {
...
export const removeUserFromSupplier = async (payload: RemoveUserPayload) => {
  try {
    const response = await AxiosClient.post(
      ENDPOINTS.supplier.REMOVE_USER_FROM_SUPPLIER,
      payload
    ); // Or AxiosClient.delete if appropriate
    return response.data;
  } catch (error) {
    if (error instanceof AxiosError) {
      return (
        error.response?.data || {
          message: "An error occurred while removing user from supplier",
          status: false,
        }
      );
    } else {
...
export const getSupplierPriceList = async (supplierDetailId: string) => {
  try {
    const response = await AxiosClient.get(
      ENDPOINTS.supplier.GET_PRICE_LIST(supplierDetailId)
    );
    return response.data;
  } catch (error) {
    if (error instanceof AxiosError) {
      return (
        error.response?.data || {
          message: "An error occurred while fetching price list",
          status: false,
        }
      );
    } else {
...
export const updateSupplierPriceList = async (
  supplierDetailId: string,
  payload: [{ item_id: string; price: number }]
) => {
  ...
  try {
    ...
    if (error instanceof AxiosError) {
      return (
        error.response?.data || {
          message: "An error occurred while updating price list",
          status: false,
        }
      );
    } else {
