...
const KeymanSignupComponent: React.FC = () => {
  ...
  const handleSubmit = async (values: SignupFormValues) => {
    ...
    if (response.status) {
      ...
      notify.error(errorMessage, "Registration Failed");
    }
    setLoading(false);
  };
    ...
    <div className="min-h-screen flex flex-col md:flex-row  justify-evenly ">
      <section className=" text-white flex flex-col items-center justify-start bg-keyman-accent border-2  shadow-lg p-8 md:p-16  min-h-[360px] md:min-h-screen w-full md:w-1/2 max-w-full ">
        <Box className="p-8 md:p-12 flex flex-col justify-center">
          <Title
            order={1}
            fw={700}
            className="text-transparent bg-clip-text bg-gradient-to-r from-orange-300 to-orange-400"
          >
            KEYMAN Stores
          </Title>
        </Box>
        <Title fw={500} order={2} mb="md" c="white">
      ...
      <div className="w-full md:w-1/2  max-w-full bg-gray-50 p-8 md:px-16 md:py-4 ">
        ...
        <div className="text-center mb-8">
          ...
          <Title order={1} fw={700} className="text-gray-900 mb-4">
            Create Your Account
          </Title>
          <Group gap={4} justify="center">
            ...
            <Link
              href="/account/login"
              className=" text-keyman-green font-semibold  hover:keyman-green-hover transition-colors duration-200"
            >
              SIGN IN
            </Link>
          </Group>
