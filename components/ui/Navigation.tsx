    ...
    <nav className={`fixed   w-full z-50 transition-all duration-300 px-4  md:px-8 ${darkMode ? 'bg-gray-900/95' : 'bg-white/95'} backdrop-blur-md border-b ${darkMode ? 'border-gray-800' : 'border-gray-200'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <Link href={isAccountPage ? "/" : "/keyman/dashboard"} className="flex items-center space-x-2">
            <div className="w-10 h-10  rounded-lg flex items-center justify-center">
              <Image src="/keyman_logo.png" alt="Keyman Logo" width={40} height={40} className="rounded-lg" />
            </div>
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            {isAccountPage  ? null :    <Link href="/account/sign-up" className="bg-gradient-to-r from-[#3D6B2C] to-[#4CAF50] text-white px-6 py-2 rounded-lg font-medium hover:shadow-lg transform hover:scale-105 transition-all duration-200">
              Sign Up
            </Link>}
          </div>
