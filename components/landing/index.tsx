        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-8 animate-fade-in">
            <div className="space-y-4">
              <h1 className={`text-4xl md:text-6xl font-bold ${darkMode ? 'text-white' : 'text-gray-900'} leading-tight`}>
                Quality made
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#3D6B2C] to-[#4CAF50]"> Affordable</span>
              </h1>
              <p className={`text-lg md:text-xl ${darkMode ? 'text-gray-300' : 'text-gray-600'} max-w-lg`}>
                Connect with trusted construction products and professionals. Build your dream project with confidence and ease.
              </p>
            </div>
...
const RequestOrderSection: React.FC<{ darkMode: boolean }> = ({ darkMode }) => {
  return (
    <section className={`py-16 px-4 sm:px-6 lg:px-8 ${darkMode ? 'bg-gray-900' : 'bg-gradient-to-br from-green-50 to-emerald-50'}`}>
      <div className="max-w-7xl mx-auto">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            ...
            <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
              Get accurate quotations for your construction projects. Our AI-powered system helps you build comprehensive project lists and connect with trusted suppliers.
            </p>
            ...
            <div className="space-y-4">
              <div className="flex items-center space-x-3">
                ...
                <span className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Add items to your cart with AI assistance</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-[#4CAF50] rounded-full flex items-center justify-center">
                  <CheckCircle className="w-5 h-5 text-white" />
                </div>
                <span className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Get competitive quotes from verified suppliers</span>
              </div>
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 bg-[#4CAF50] rounded-full flex items-center justify-center">
                  <CheckCircle className="w-5 h-5 text-white" />
                </div>
                <span className={`${darkMode ? 'text-gray-300' : 'text-gray-700'}`}>Track your project progress in real-time</span>
              </div>
            </div>
            ...
            <div className={`${darkMode ? 'bg-gray-800' : 'bg-white'} rounded-3xl p-8 shadow-2xl transform hover:rotate-1 transition-transform duration-500`}>
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h3 className={`text-xl font-semibold ${darkMode ? 'text-white' : 'text-gray-900'}`}>Project Cart</h3>
                ...
                <div className="space-y-4">
                  {['Cement bags (50kg)', 'Steel bars (12mm)', 'Building blocks'].map((item, index) => (
    ...
    <section className={`py-16 px-4 sm:px-6 lg:px-8 ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
      <div className="max-w-4xl mx-auto text-center space-y-8">
        <div className="space-y-4">
          ...
          <p className={`text-lg ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
            Purchase tokens to place orders seamlessly. Our token system ensures secure transactions and priority access to premium suppliers.
          </p>
        </div>
            ...
            <div key={index} className={`relative ${darkMode ? 'bg-gray-700' : 'bg-gray-50'} rounded-2xl p-6 ${plan.popular ? 'ring-2 ring-[#F08C23] transform scale-105' : ''}`}>
              {plan.popular && (
                <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                  <span className="bg-[#F08C23] text-white px-4 py-1 rounded-full text-sm font-medium">Popular</span>
                </div>
              )}
              <div className="space-y-4">
